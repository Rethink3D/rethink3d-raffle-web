...
export const MissionsPage: React.FC = () => {
            ...
            <tbody className="divide-y divide-brand-border/40 text-xs font-ui font-bold text-brand-strong tracking-wider">
              {list.map((mission, index) => (
                <tr key={mission.id} className="hover:bg-brand-surface/30 transition-colors">
                  ...
                  <td className="p-4">
                    <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-brand-primary/10 border border-brand-primary/30 text-brand-primary">
                      {getMissionTypeLabel(mission.type)}
                    </span>
                  </td>
                  <td className="p-4 text-center font-mono text-brand-secondary text-sm">+{mission.reward}</td>
                    ...
                    {isGlobal ? (
                      ...
                      <button
                        ...
                        className={`px-2 py-0.5 rounded text-[10px] font-mono border cursor-pointer ${
                          mission.active
                            ? 'bg-brand-success/15 border-brand-success text-brand-success hover:bg-brand-success/25'
                            : 'bg-brand-danger/15 border-brand-danger text-brand-danger hover:bg-brand-danger/25'
                        }`}
                      >
  ...
  return (
    <div className="space-y-6 font-body">
      ...
      {/* Tabs */}
      <div className="flex gap-1 bg-brand-surface/50 border border-brand-border/40 rounded-lg p-1 w-fit">
        <button
          onClick={() => setActiveTab('campaign')}
          className={`flex items-center gap-2 px-4 py-2 rounded text-xs font-display font-bold tracking-widest uppercase transition-all cursor-pointer ${
            activeTab === 'campaign'
