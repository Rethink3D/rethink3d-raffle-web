...
export const FeedbackPage: React.FC = () => {
    ...
    const fetchFormAndQuest = async () => {
      ...
      try {
        ...
        if (formData && formData.questions) {
          ...
          const sorted = [...formData.questions].sort((a, b) => a.order - b.order);
          setQuestions(sorted);
        }
  ...
  const handleAnswerChange = (questionId: string, value: string | string[]) => {
    ...
    // If we changed a question, we should check if any other question depends on it,
    // and if the new value doesn't match the condition, we reset that answer.
    let changed = true;
    while (changed) {
      changed = false;
      for (const q of questions) {
        if (q.conditionKey && updatedAnswers[q.id] !== undefined) {
          // If parent is not visible, or doesn't match condition
  ...
  const validateForm = (): boolean => {
    ...
    questions.forEach((q) => {
      ...
      if (!visible) return; // Skip validation for hidden fields
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Prepare answers payload containing only visible questions to avoid sending stale values
